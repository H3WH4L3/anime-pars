...
def get_json_titles(title):
    ...
    anime_list = {
        ...
        if element.get("shikimori_id") is not None # Проверка на отсутствие ID
    }

    # Сортируем словарь по годам
    anime_list = dict(sorted(anime_list.items(), key=lambda x: x[1]["Год"]))

    # Находим количество серий и переводы, и вносим в основной словарь
    for key, value in anime_list.items():
        ...
        anime_list[key]["Количество серий"] = response["results"][0].get("episodes_count", 0)
